...
export default async function handler(
    ...
    if (req.method === "GET") {
        ...
        if (maxBase !== '1') {
            ...
            let startTime_q = `last("timestart_b1")  as ftime_b1`
            for (let i = 2; i <= parseInt(maxBase as string); i++) {
                ...
                startTime_q += `,last("timestart_b${i}") as ftime_b${i}`
            }
            ...
            queryStart = `SELECT ${startTime_q} FROM "Performance" WHERE ("Area" = '${(area)}') AND  ("Site" = '${(site as string)}') AND  ("Line" = '${(line as string)}') AND actual_rate = 0 `;
        }
        let data = {
            method: "post",
        ...
        // if (line == "WD2") {
            // console.log(line, newArrData, getData.data.results[0].series[0].columns, getData.data.results[0].series[0].values[0])
        // }
